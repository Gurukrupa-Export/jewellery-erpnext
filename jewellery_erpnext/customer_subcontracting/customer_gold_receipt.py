...
def set_customer_gold_rate_snapshot(doc, settings):
	...
	* a client-supplied value can never become financial truth -- whatever arrives over the
	...
	Once the receipt is submitted ``before_validate`` no longer runs, so the snapshot is
	frozen permanently. Cancellation does not clear it: a cancelled receipt must still show
	the rate it originally used. An amendment carries no snapshot (the fields are
	``no_copy``) and resolves afresh against its own posting date.
