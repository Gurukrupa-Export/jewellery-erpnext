...
class SketchOrderForm(Document):
        ...
        for row in self.order_details:
            if row.subcategory:
                ...
                if row.category != parent_category:
                    frappe.throw(
                        _("Category & Sub Category mismatched in row #{0}").format(
                            row.idx
                        )
                    )
    ...
    def cancel_linked_sketch_orders(self):
        ...
        frappe.db.set_value(
            "Sketch Order Form", self.name, "workflow_state", "Cancelled"
        )
        self.reload()
    ...
    if created_orders:
        frappe.msgprint(
            _("The following {0} were created: {1}").format(
                frappe.bold(_("Sketch Orders")), "<br>" + ", ".join(created_orders)
            )
        )
...
def calculate_ibm_delivery(base_datetime, ibm_time_value, shift_start, shift_end):
    ...
    remaining_hours = max(0, (shift_end_dt - base_datetime).total_seconds() / 3600)
...
def make_sketch_order(
    ...
    def set_missing_values(source, target):
        ...
        apply_order_criteria_dates(
            parent_doc, target, order_criteria, department_shifts
        )
...
def create_purchase_order(doc):
    ...
    frappe.msgprint(
        _("The following {0} is created: {1}").format(
