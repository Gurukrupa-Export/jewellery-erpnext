...
def make_department_stock_entry(self, **kwargs):
	...
	# frappe.throw(f"{new_se_doc.name}")
	# self.db_set("custom_reserve_se", new_se_doc.name)
	frappe.db.set_value("Material Request", self.name, "custom_reserve_se", new_se_doc.name)
...
@frappe.whitelist()
def update_department(docname, new_department):
    doc = frappe.get_doc("Material Request", docname)
    current_department = doc.custom_department
    if new_department == current_department:
        # Increment counter without saving doc using direct DB update
        current_count = doc.get('custom_custom_counter') or 0
        new_count = current_count + 1
        frappe.db.set_value("Material Request", docname, "custom_custom_counter", new_count)
    else:
        # Update department and reset counter
        frappe.db.set_value("Material Request", docname, {
            "custom_department": new_department,
            "custom_custom_counter": 1
        })
    frappe.db.commit()  # Ensure changes are committed immediately
    return "Updated"
