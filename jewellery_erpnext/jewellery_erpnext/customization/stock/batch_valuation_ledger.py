...
class BatchValuationLedger:
    ...
    #         )
    #         .groupby(parent.warehouse, parent.item_code, child.batch_no)
    #     )
    ...
    def combine_conditions(self, conditions):
        """
        Combine a list of conditions in a balanced manner (using bitwise OR)
        to reduce the maximum recursion depth required when chaining many conditions together.
        """
        if not conditions:
            return None

        # Combine pairwise until a single condition remains
        while len(conditions) > 1:
            new_conditions = []
            for i in range(0, len(conditions), 2):
                if i + 1 < len(conditions):
                    new_conditions.append(conditions[i] | conditions[i+1])
                else:
    ...
    def get_historical_batch_ledger_data(self, sles: list, exclude_voucher_no: str):
        ...
        # Combine conditions in a balanced manner
