...
class CustomSubmissionQueue(SubmissionQueue):
	def insert(self, to_be_queued_doc: Document, action: str):
		...
		if self.ref_doctype in ["Employee IR","Department IR","Product Certification","Stock Entry"] and queue:
			frappe.msgprint(
	...
	def after_insert(self):
		if self.ref_doctype in ["Employee IR","Department IR","Product Certification","Stock Entry"]:
			self.queue_action(
