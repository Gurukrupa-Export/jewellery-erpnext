...
def before_validate(self, method):
	...
	if self.sales_type == "Hybrid":
		has_real_items = any(
			row.item_code != "Subcontracting Charges" for row in self.items
		)
		if has_real_items:
			self.items = [
...
def on_submit(self, method):
	...
	if self.is_return and self.sales_type == "Hybrid":
		...
		custom_item, hsn_code, uom = frappe.db.get_value(
			"E Invoice Item",
			{"is_for_labour": 1, "metal_purity": bom_doc.metal_touch},
			["name", "hsn_code", "uom"],
		)
		new_si.append(
			"invoice_item",
		...
		try:
			...
			frappe.log_error(
				title="New SI Creation Failed", message=frappe.get_traceback()
			)
...
def xl_preview_sales_invoice(docname):
	# openpyxl is only needed by this one export, so it is imported lazily rather than at
	# module level — the module is loaded on every Sales Invoice save.
		...
		if row.bom and not self.item_same_as_above:
			...
			if self.sales_type != "Certification":
				...
				row.making_amount = (
					0
					if self.sales_type == "Hybrid"
					else flt(sum(r.making_amount for r in finding_rows))
...
def update_einvoice_items(
	...
	if invoice_data:
		...
		for item in item_flags:
			if item.is_for_diamond:
				weight = 1
			elif item.is_for_metal:
				weight = 2
			elif item.is_for_finding:
				weight = 3
			elif item.is_for_gemstone:
				weight = 4
			elif item.is_for_making:
				weight = 5
			elif item.is_for_finding_making:
				weight = 6
			elif item.is_for_hallmarking:
				weight = 7
			elif item.is_for_labour:
				weight = 8
			elif item.is_for_repair:
				weight = 9
			elif item.is_for_certification:
				weight = 10
			else:
				weight = 99
			sort_weights[item.name] = weight

		sorted_keys = sorted(item_names, key=lambda k: sort_weights.get(k, 99))
	else:
		sorted_keys = []
			...
			if einvoice_item in invoice_data:
				if so_item:
					...
					invoice_data[einvoice_item]["amount"] = round(
						flt(invoice_data[einvoice_item]["amount"]) + flt(amount),
						precision,
					)
					invoice_data[einvoice_item]["qty"] = flt(
						invoice_data[einvoice_item]["qty"]
					) + flt(qty)
			else:
