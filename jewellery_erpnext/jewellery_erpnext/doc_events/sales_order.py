...
def after_mapping(self, method=None, source_doc=None):
    ...
    if source_doc.get("custom_sales_type"):
...
def validate_sales_type(self):
    ...
    for r in self.items:
        if not r.prevdoc_docname:
            continue
        if r.prevdoc_docname not in seen:
            seen[r.prevdoc_docname] = (
                frappe.db.get_value(
                    "Quotation",
                    r.prevdoc_docname,
        ...
        source = seen[r.prevdoc_docname]
        if source.custom_sales_type:
