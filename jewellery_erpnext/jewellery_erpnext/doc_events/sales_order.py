...
# 	# validate_items(self)
# 	# if self.workflow_state=='Invoice Generated' :
# 	validate_item_dharm(self)
# 	# self.calculate_taxes_and_totals()
# 	# calculate_gst_rate(self)
# 	# if self.workflow_state=='BOM Created' :
...
# 							"metal_purity",
# 						)
# 						frappe.throw(f'{customer_metal_purity}')
# 						calculated_gold_rate = (float(customer_metal_purity) * self.gold_rate_with_gst) / (100 + int(gold_gst_rate))
# 						if doc.metal_and_finding_weight < threshold:
...
# 								d.diamond_rate_for_specified_quantity = round(d.quantity * ( d.handling_rate + d.total_diamond_rate), 2)
# 								d.weight_per_pcs =(d.quantity/d.pcs)
# 								if 0.001 < (d.quantity/d.pcs) > 0.005:
# 									if len(str(d.weight_per_pcs)) >4 and str(d.weight_per_pcs)[4] == '9':
# 										d.weight_per_pcs = float(str(d.weight_per_pcs)[:5])
...
# #         if not ref_customer:
# #             sales_order  = frappe.get_value("Parent Manufacturing Order", snc, "sales_order")
# #             ref_customer = frappe.db.get_value("Sales Order", sales_order, "ref_customer")
...
# #         cctx.billing_currency   = frappe.get_value("Customer", ref_customer, "default_currency")
...
# #     }
# #     if self.company == "KG GK Jewellers Private Limited":
# #         filters["customer"] = cctx.reference_customer
# #     if self.company == "Gurukrupa Export Private Limited" and ctx.customer_group == "Internal":
# #         filters["customer"] = cctx.reference_customer_c2c

# #     mc = frappe.get_all("Making Charge Price", filters=filters, fields=["name"], limit=1)
# #     if not mc:
# #         frappe.throw(
# #             f'Create a valid Making Charge Price for Customer: {filters["customer"]}, '
...
# #     return mc_name, sub_info, threshold
...
# #                             int(sf_row.from_diamond) <= int(diamond_pcs) <= int(sf_row.to_diamond)
# #                         ):
# #                             find_data = sf_row
# #                             break
...
# #         customer_metal_purity = frappe.db.get_value(
# #             "Metal Criteria",
# #             {"parent": self.customer, "metal_type": f.metal_type, "metal_touch": f.metal_touch},
# #             "metal_purity",
# #         )
# #         calculated_gold_rate = (
# #             float(customer_metal_purity) * self.gold_rate_with_gst
...
# #         elif gemstone_price_list_customer == "Diamond Range" and ctx.customer_group != "Retail":
# #             gpc = frappe.get_all(
# #                 "Gemstone Price List",
# #                 filters={
# #                     "customer":        self.customer,
...
# #                 rate    = 0
# #                 quality_map = {
# #                     "Precious":      ("outwork_precious_percentage",      "precious_percentage"),
# #                     "Semi-Precious": ("outwork_semi_precious_percentage",  "semi_precious_percentage"),
# #                     "Synthetic":     ("outwork_synthetic_percentage",      "synthetic_percentage"),
# #                 }
...
# #                     ):
# #                         outwork_field, standard_field = quality_map.get(gem.gemstone_quality, (None, None))
# #                         rate = (
# #                             getattr(mul, outwork_field, 0) if gem.is_customer_item
...
# #         price_list_type = result[0]["diamond_price_list"]
# #         common_filters  = {
# #             "price_list":      "Standard Selling",
# #             "price_list_type": price_list_type,
...
# #     ccp = frappe.db.get_all("Customer Certification Price", filters={"customer": self.customer}, limit=1)
# #     if ccp:
# #         ccp_doc = frappe.get_doc("Customer Certification Price", ccp[0].name)
...
# #     ccp = frappe.db.get_all(
# #         "Customer Certification Price", filters={"customer": self.customer}, limit=1
# #     )
# #     if ccp:
# #         ccp_doc = frappe.get_doc("Customer Certification Price", ccp[0].name)
# #         doc.certification_amount = (
...
# #         customer_metal_purity = _get_metal_purity(self.customer, f.metal_type, f.metal_touch)
# #         calculated_gold_rate  = (
# #             float(customer_metal_purity) * self.gold_rate_with_gst
# #         ) / (100 + int(ctx.gold_gst_rate))
...
# #     # Cache gemstone price list type per customer
...
# #     fields = [
# #         "bom", "bom_no", "rate", "amount",
# #         "gold_bom_rate", "diamond_bom_rate", "gemstone_bom_rate",
# #         "other_bom_rate", "making_charge",
# #         "custom_company_rm_weight", "custom_customer_weight",
# #     ]
...
# #             "custom_company_rm_weight": getattr(row, "custom_company_rm_weight", 0) or 0,
...
# #     set_clause = ", ".join(
# #         f"`{f}` = CASE `name` {' '.join(cases[f])} END"
# #         for f in fields
# #     )
...
# #  MODULE-LEVEL CACHES
# # ──────────────────────────────────────────────────────────────────
# _company_ctx_cache         = {}   # (tag, key)                          → cctx
# _making_charge_cache       = {}   # (customer, metal_type, setting_type, gold_rate, touch) → (mc_name, sub_info, threshold)
# _metal_purity_cache        = {}   # (customer, metal_type, metal_touch) → metal_purity
# _ccp_cache                 = {}   # customer                            → ccp_doc | None
# _gemstone_pl_cache         = {}   # customer                            → price_list_type

# _gold_rate_cache           = {}   # (customer, metal_type, touch, gold_rate_with_gst, gold_gst_rate) → calculated_gold_rate

# # "fetch-all-then-filter" caches
# _making_charge_list_cache  = {}   # (customer, metal_type, setting_type, gold_rate) → all matching MC Price rows
# _metal_purity_list_cache   = {}   # customer                            → all Metal Criteria rows
# _gemstone_price_list_cache = {}   # customer                            → all Gemstone Price List rows
# _gemstone_price_doc_cache  = {}   # gpc doc name                        → full frappe doc (Diamond Range)
# _diamond_pl_list_cache     = {}   # customer_key                        → all Diamond Price List Table rows
# _diamond_price_cache       = {}   # (customer_key, pl_type, shape, quality, diamond_type, sieve/weight/mm) → rate row
...
#     _ccp_cache.clear()
#     _gemstone_pl_cache.clear()
#     _gold_rate_cache.clear()
#     _making_charge_list_cache.clear()
#     _metal_purity_list_cache.clear()
#     _gemstone_price_list_cache.clear()
#     _gemstone_price_doc_cache.clear()
#     _diamond_pl_list_cache.clear()
#     _diamond_price_cache.clear()
...
# def _get_all_metal_purity(customer):
#     """
#     Fetch every Metal Criteria row for a customer in one query,
#     then cache. Subsequent lookups filter in Python.
#     """
#     if customer not in _metal_purity_list_cache:
#         _metal_purity_list_cache[customer] = frappe.db.get_all(
#             "Metal Criteria",
#             filters={"parent": customer},
#             fields=["metal_type", "metal_touch", "metal_purity"],
#         )
#     return _metal_purity_list_cache[customer]


# def _get_metal_purity(customer, metal_type, metal_touch):
#     key = (customer, metal_type, metal_touch)
#     if key not in _metal_purity_cache:
#         rows = _get_all_metal_purity(customer)
#         match = next(
#             (r for r in rows
#              if r.metal_type == metal_type and r.metal_touch == metal_touch),
#             None,
#         )
#         _metal_purity_cache[key] = match.metal_purity if match else None
#     return _metal_purity_cache[key]


# def _get_calculated_gold_rate(customer, metal_type, metal_touch,
#                                gold_rate_with_gst, gold_gst_rate):
...
#         _gold_rate_cache[key] = (
#             float(purity) * gold_rate_with_gst
#         ) / (100 + int(gold_gst_rate))
#     return _gold_rate_cache[key]


# def _get_all_making_charge_rows(customer, metal_type, setting_type, gold_rate_with_gst):
#     """
#     Fetch every Making Charge Price row for (customer, metal_type,
#     setting_type, gold_rate range) in ONE query. Subcategory filtering
#     is done in Python inside _get_making_charge.
#     """
#     key = (customer, metal_type, setting_type, gold_rate_with_gst)
#     if key not in _making_charge_list_cache:
#         mc_list = frappe.get_all(
#             "Making Charge Price",
#             filters={
#                 "customer":       customer,
#                 "metal_type":     metal_type,
#                 "setting_type":   setting_type,
#                 "from_gold_rate": ["<=", gold_rate_with_gst],
#                 "to_gold_rate":   [">=", gold_rate_with_gst],
#             },
#             fields=["name", "metal_touch"],
#         )
#         _making_charge_list_cache[key] = mc_list
#     return _making_charge_list_cache[key]


# def _get_gemstone_price_list(customer):
#     """
#     Fetch ALL Gemstone Price List rows for a customer once.
#     All subsequent lookups filter in Python.
#     """
#     if customer not in _gemstone_price_list_cache:
#         _gemstone_price_list_cache[customer] = frappe.get_all(
#             "Gemstone Price List",
#             filters={"customer": customer},
#             fields=[
#                 "name", "price_list_type", "rate", "handling_rate",
#                 "outwork_handling_charges_rate", "per_pc_or_per_carat",
#                 "cut_or_cab", "gemstone_type", "stone_shape", "gemstone_grade",
#                 "is_retail_customer",
#             ],
#         )
#     return _gemstone_price_list_cache[customer]


# def _get_diamond_pl_list(customer_key):
#     """
#     Fetch ALL Diamond Price List Table rows for a customer once.
#     """
#     if customer_key not in _diamond_pl_list_cache:
#         _diamond_pl_list_cache[customer_key] = frappe.db.sql(
#             """SELECT diamond_price_list, diamond_shape
#                FROM `tabDiamond Price List Table`
#                WHERE parent = %s""",
#             customer_key, as_dict=True,
#         )
#     return _diamond_pl_list_cache[customer_key]
...
# def _get_bom_context(self):
#     gold_gst_rate = frappe.db.get_single_value("Jewellery Settings", "gold_gst_rate")
#     customer_group, precision, metal_precision, stone_precision,precision_for_gross_weight,precision_for_net_weight = frappe.db.get_value(
#         "Customer",
#         self.customer,
#         ["customer_group", "custom_precision_variable",
#          "custom_precision_for_metal", "custom_precision_for_stone","custom_precision_for_gross_weight","custom_precision_for_net_weight"],
#     )
#     return frappe._dict(
#         gold_gst_rate=gold_gst_rate,
#         customer_group=customer_group,
...
#     if cache_key not in _making_charge_cache:
#         # ── Use fetch-all cache: filter by touch in Python ──────
#         mc_list = _get_all_making_charge_rows(
#             effective_customer, doc.metal_type,
#             doc.setting_type,  self.gold_rate_with_gst,
#         )
#         mc = next((r for r in mc_list if r.metal_touch == touch), None)
#         if not mc:
#             frappe.throw(
#                 f'Create a valid Making Charge Price for Customer: {effective_customer}, '
#                 f'Metal Type: {touch}, Setting Type: {doc.setting_type}'
...
#         mc_name  = mc["name"]
#         # frappe.throw(f"{mc_name}")
#         # ── Fetch ALL subcategory rows for this MC Price in one go ──
#         all_sub_rows = frappe.db.get_all(
#             "Making Charge Price Item Subcategory",
#             filters={"parent": mc_name},
#             fields=[
#                 "subcategory", "rate_per_gm", "rate_per_pc",
#                 "supplier_fg_purchase_rate", "wastage","wastage_per_pcs",
#                 "subcontracting_rate", "subcontracting_wastage",
#                 "rate_per_gm_threshold", "to_diamond", "from_diamond",
#             ],
#         )

#         # Filter by item_subcategory in Python
#         sub_rows = [r for r in all_sub_rows if r.subcategory == doc.item_subcategory]
#         if not sub_rows:
#             frappe.throw(
#                 f'No subcategory "{doc.item_subcategory}" found in Making Charge Price {mc_name}'
#             )
...
# # ──────────────────────────────────────────────────────────────────
# #  FINDING CHARGE
# #  Same "fetch-all-then-filter" pattern for finding subcategory rows
# # ──────────────────────────────────────────────────────────────────

# # Cache: mc_name → all Finding Subcategory rows for that MC Price doc
# _finding_sub_cache = {}   # mc_name → list of all finding subcategory rows
...
#             filters={"parent": mc_name},
#             fields=[
#                 "subcategory", "rate_per_gm", "rate_per_pc", "wastage","wastage_per_pcs",
#                 "supplier_fg_purchase_rate", "subcontracting_rate",
#                 "subcontracting_wastage",
#             ],
#         )
...
#     match = next((r for r in all_finding_rows if r.subcategory == finding_type), None)
#     if match:
#         return match
...
#     if not hasattr(doc, "metal_detail"):
#         return
#     max_metal_row = max(doc.metal_detail, key=lambda s: s.quantity, default=None)
#     for s in doc.metal_detail:
#         making_charges_on = frappe.db.get_value('Customer',self.customer, 'compute_making_charges_on')
#         diamond_quantity = sum(d['quantity'] for d in doc.diamond_detail)
#         # frappe.throw(f"hii , {diamond_quantity}")
#         _, sub_info, threshold = _get_making_charge(self, doc, s.metal_touch, ctx, cctx)
...
#     # Fetch ALL price list rows for customer once
#     all_gpc = _get_gemstone_price_list(self.customer)
...
#             gpc_row = next((
#                 r for r in all_gpc
#                 if r.price_list_type     == gemstone_price_list_customer
#                 and r.per_pc_or_per_carat == gem.get("per_pc_or_per_carat")
#                 and r.cut_or_cab          == gem.get("cut_or_cab")
#                 and r.gemstone_type       == gem.get("gemstone_type")
#                 and r.stone_shape         == gem.get("stone_shape")
#                 and r.gemstone_grade      == gem.get("gemstone_grade")
#             ), None)
#             if not gpc_row:
#                 frappe.msgprint(
#                     f'No Gemstone Price List found: {gem.get("per_pc_or_per_carat")}, '
#                     f'{gem.get("cut_or_cab")}, {gem.get("gemstone_type")}, '
#                     f'{gem.get("stone_shape")}'
#                 )
#                 continue

#             rate = (
#                 gpc_row["outwork_handling_charges_rate"]
#                 if gem.is_customer_item
#                 else gpc_row["rate"] or 0
#             )
#             gem.total_gemstone_rate = round(rate, 2)
#             gem.gemstone_rate_for_specified_quantity = round(
#                 float(rate) * float(gem.quantity)
...
#             gpc_row = next((
#                 r for r in all_gpc
#                 if r.is_retail_customer   == 1
#                 and r.price_list_type     == gemstone_price_list_customer
#                 and r.per_pc_or_per_carat == gem.get("per_pc_or_per_carat")
#                 and r.cut_or_cab          == gem.get("cut_or_cab")
#                 and r.gemstone_type       == gem.get("gemstone_type")
#                 and r.stone_shape         == gem.get("stone_shape")
...
#             # Cache the full doc (multiplier child rows) by name
#             if gpc_row.name not in _gemstone_price_doc_cache:
#                 _gemstone_price_doc_cache[gpc_row.name] = frappe.get_doc(
#                     "Gemstone Price List", gpc_row.name
#                 )
#             gpc_doc = _gemstone_price_doc_cache[gpc_row.name]
...
#     # Fetch ALL diamond price list table rows for customer once
#     all_dl = _get_diamond_pl_list(customer_key)
...
#         # ── Filter in Python instead of SQL per row ───────────────
#         dl_row = next((r for r in all_dl if r.diamond_shape == d.stone_shape), None)
#         if not dl_row:
#             d.total_diamond_rate                  = 0
#             d.diamond_rate_for_specified_quantity = 0
#             continue

#         price_list_type = dl_row["diamond_price_list"]
#         common_filters  = {
#             "price_list":      "Standard Selling",
#             "price_list_type": price_list_type,
#             "customer":        customer_key,
...
# #         + doc.gemstone_bom_amount + doc.finding_bom_amount
# #         + doc.total_wastage_amount
# #         + sum(flt(r.wastage_amount) for r in doc.get("finding_detail", [])),ctx.precision
# #     )
...
# #     )
# #     doc.metal_and_finding_weight             = round(flt(doc.gross_weight) - flt(doc.total_gemstone_weight_in_gms) - flt(doc.total_diamond_weight_in_gms) , ctx.precision_for_net_weight)
# #     doc.gold_to_diamond_ratio = (
# #         flt(doc.metal_and_finding_weight) / flt(doc.diamond_weight) if doc.diamond_weight else 0
# #     )
# #     doc.diamond_ratio = (
...
# #     # row.amount=total_amount
# #     # row.rate-total_amount
# #     # self.total += round(row.amount,ctx.precision)
    
# #     # frappe.throw(f"hii{ctx.precision},{total_amount},{self.total}")
# #     if self.sales_type == "Repairing":
# #         total_amount = doc.total_bom_amount
# #     row.item_code         = item_code
# #     row.serial_no         = serial_no
...
# #     # frappe.throw(f"{row.gold_bom_rate}")
...
# #     # self.total = round(self.total,precision)
# #     # frappe.throw(f"{self.total}")
...
# 	# frappe.throw(f"{_update_bom_totals}")
#     if not row.quotation_bom:
#         if not row.bom:
#             return
...
# def create_new_bom1(self):
#     """
#     Process all BOM rows synchronously with aggressive caching.

#     DB call reduction summary
#     ─────────────────────────────────────────────────────────────
#     _get_bom_context          : 1 call total (gold_gst_rate + customer fields)
#     _get_company_context      : 1 call per serial_no / SO   (was 1 per row)
#     _get_all_making_charge_rows: 1 call per (customer, metal, setting, rate)
#                                 (was 1 per row; touch filtered in Python)
#     Making Charge subcategory : 1 call per mc_name          (was 1 per row)
#     _get_all_finding_sub_rows : 1 call per mc_name          (was 1 per finding row)
#     _get_all_metal_purity     : 1 call per customer         (was 1 per metal/finding sub-row)
#     _get_calculated_gold_rate : computed once per combo, cached
#     _get_gemstone_price_list  : 1 call per customer         (was 1 per gem row)
#     _gemstone_price_doc_cache : 1 frappe.get_doc per unique gpc doc
#     _get_diamond_pl_list      : 1 call per customer_key     (was 1 SQL per diamond row)
#     _ccp_cache                : 1 call per customer
#     _bulk_update_child_rows   : 1 UPDATE for all rows       (was N set_value calls)
#     ─────────────────────────────────────────────────────────────
#     """
#     _clear_caches()
...
# 		precision = frappe.db.get_value('Customer',self.customer,'custom_precison_variable')
# 		self.total = round(self.total,precision)
...
# 								multiplied_qty = metal.quantity * item.qty
# 								metal_making_amount = round(metal.making_rate * multiplied_qty  + (metal.wastage_amount * item.qty) , precision)
...
# 								diamond_amount = round(diamond.diamond_rate_for_specified_quantity , precision)
...
# 								gemstone_amount = round(gemstone.gemstone_rate_for_specified_quantity , precision)
...
# 			val["rate"] = round(
#         (val["amount"] / val["qty"]) if val["qty"] else 0,precision)
...
# 			val["amount"] = round(val["amount"], precision)
# 			val["rate"] = round(
# 				(val["amount"] / val["qty"]) if val["qty"] else 0,
# 				precision
# 			)
# 			# frappe.throw(f"{val["qty"]}")
# 			self.append("custom_invoice_item", val)
...
def on_cancel(self, method):
	for row in self.items:
		...
		if row.bom:
			tb = frappe.get_doc("Tracking Bom", row.bom)
			if tb.docstatus == 1:
				tb.cancel()
...
def _get_calculated_gold_rate(customer, metal_type, metal_touch,gold_rate_with_gst, gold_gst_rate):
    key = (customer, metal_type, metal_touch, gold_rate_with_gst, gold_gst_rate)
    if key not in _gold_rate_cache:
        purity = _get_metal_purity(customer, metal_type, metal_touch)
        _gold_rate_cache[key] = (
...
def _reconcile_metal_weights(doc, ctx):
	"""
	After gross/net weights are computed, adjust the highest-quantity metal row
	so that sum(metal + non-chain finding quantities) == metal_and_finding_weight.
	...
	highest_row.quantity   = round(flt(highest_row.quantity) - diff, precision)
...
def _update_bom_totals(self, doc, row, ctx, item_code, serial_no):
    ...
    # ── 7. Physical tag gross weight — preserve what was stored ──
    # doc.gross_weight already holds the physical tag weight set
    # during BOM creation from the Serial Number Creator.
    # We do NOT overwrite it here from component sums because the
    # measured piece weight is the source of truth for net_wt.
    # It will be recomputed below (step 7b) after metal rows are
    # reconciled so it reflects the adjusted quantities.
    stored_gross_weight = flt(doc.gross_weight)   # physical tag weight

    # ── 8. NET METAL WEIGHT — derived from physical gross weight ─
    # Use the stored (tag) gross weight, not a sum of current rows,
    # so that small discrepancies between design weight and the
    # actual measured piece do not corrupt the net weight.
    if making_charges_on == 'Diamond Inclusive':
        ...
        # Diamond Exclusive — subtract diamond grams from net
        doc.metal_and_finding_weight = round(
            stored_gross_weight
            - flt(doc.total_diamond_weight_in_gms)
            - flt(doc.total_gemstone_weight_in_gms)
    ...
    # ── 8b. Reconcile metal row quantities with net weight ───────
    # Adjusts the highest-quantity metal row so that the sum of
    # metal rows matches metal_and_finding_weight exactly.
    # Finding rows are never touched.
    _reconcile_metal_weights(doc, ctx)

    # ── 7b. Recompute gross weight from reconciled metal rows ────
    # Now that metal rows have been adjusted, gross_weight is
    # recalculated so it stays consistent with the row quantities.
    doc.gross_weight = round(
    ...
    doc.diamond_ratio = (
        ...
        if doc.total_diamond_pcs else 0
    )
    doc.metal_to_diamond_ratio_excl_of_finding = (
        flt(doc.metal_weight) / flt(doc.diamond_weight)
        if doc.diamond_weight else 0
    )
...
def _process_single_row(self, row, ctx):
	...
	# Initial total weight (raw, pre-reconcile) — used by threshold checks
	# inside _process_metal_detail1 / _process_finding_detail1.
	# _update_bom_totals will overwrite this with the physically-tagged value.
	doc.metal_and_finding_weight = (
	...
	if not row.bom and frappe.db.exists("Tracking Bom", row.custom_tracking_bom):
