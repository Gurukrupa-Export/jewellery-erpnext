		...
		if not row.quotation_bom:
			...
			if row.bom:
				...
				if hasattr(doc, "gemstone_detail"):
					for gem in doc.gemstone_detail or []:
						...
						if (
							...
							if (
								gemstone_price_list_customer == "Fixed"
								and customer_group != "Retail"
							):
								gpc = frappe.get_all(
									"Gemstone Price List",
									filters={
								...
								if not gpc:
									frappe.msgprint(
										f"No Multiplier Price List found ,{gemstone_price_list_customer}"
									)
								else:
									gpc_doc = frappe.get_doc(
										"Gemstone Price List", gpc[0].name
									)
									multiplier_rows = gpc_doc.get("gemstone_multiplier")
									rate = 0
									for mul in multiplier_rows:
										if mul.gemstone_type == gem.gemstone_type and (
											float(doc.diamond_weight)
											>= float(mul.from_weight)
											and float(doc.diamond_weight)
											<= float(mul.to_weight)
										):
											if gem.is_customer_item:
												if gem.gemstone_quality == "Precious":
													rate = (
														mul.outwork_precious_percentage
													)

												elif (
													gem.gemstone_quality
													== "Semi-Precious"
												):
													rate = mul.outwork_semi_precious_percentage

												elif (
													gem.gemstone_quality == "Synthetic"
												):
													rate = (
														mul.outwork_synthetic_percentage
													)
											else:
												if gem.gemstone_quality == "Precious":
							...
							##############
								...
								if not gpc:
									frappe.throw(
										"No Retail Multiplier Price List found"
									)
				...
				if hasattr(doc, "metal_detail"):
					for s in doc.metal_detail:
						...
						if (
							...
							filters[
								"customer"
							] = refrence_customer_for_company_to_company
						mc = frappe.get_all(
							"Making Charge Price",
							filters=filters,
							fields=["name"],
						...
						if not mc:
							frappe.throw(
								f"""Create a valid Making Charge Price for Customer: {filters["customer"] }, Metal Type:{doc.metal_touch} "Setting Type":{doc.setting_type} """
							)
						...
						threshold = (
							2
							if sub_info.get("rate_per_gm_threshold") == 0
							else sub_info.get("rate_per_gm_threshold")
						)
						...
						if doc.metal_and_finding_weight < threshold:
							for s_row in sub:
								if s_row.from_diamond:
									...
									if (
										int(s_row.from_diamond)
										<= int(diamond_pcs)
										<= int(s_row.to_diamond)
									):
										sub_info = s_row
						...
						if (
							...
							# frappe.throw("hii")
								...
								customer_metal_purity = frappe.db.sql(
									f"""select metal_purity from `tabMetal Criteria` where parent = '{self.customer}' and metal_type = '{s.metal_type}' and metal_touch = '{s.metal_touch}'""",
									as_dict=True,
								)[0]["metal_purity"]
								calculated_gold_rate = (
									float(customer_metal_purity)
							...
							if s.is_customer_item:
								...
								if billing_currency == "USD":
									...
									s.rate = s.se_rate
									s.making_rate = (
										sub_info.get("supplier_fg_purchase_rate", 0)
										* exchange_rate
									)
								else:
									s.rate = s.se_rate
							...
							if not mc:
								frappe.throw(
									f"""Create a valid Making Charge Price for Customer: {self.customer}, Metal Type:{doc.metal_type} "Setting Type":{doc.setting_type} """
								)
							mc_name = mc[0]["name"]
							...
							if doc.metal_and_finding_weight < threshold:
								...
								wastage_rate_value = (
									0  # or adjust if wastage applies for rate_per_pc
								)
							else:
					...
					doc.total_metal_amount = sum(
						float((r.amount)) for r in doc.get("metal_detail", [])
					)

					doc.total_wastage_amount = sum(
						float((r.wastage_amount)) for r in doc.get("metal_detail", [])
					)
					doc.total_making_amount = sum(
						float((r.making_amount)) for r in doc.get("metal_detail", [])
					)
				...
				if hasattr(doc, "finding_detail") and doc.finding_detail:
					...
					for f in doc.finding_detail:
						...
						if finding_type not in finding_cache:
							...
							if not find:
								...
								threshold = (
									2
									if find_data.get("rate_per_gm_threshold") == 0
									else find_data.get("rate_per_gm_threshold")
								)
						...
						calculated_gold_rate = (
							float(f.metal_purity) * self.gold_rate_with_gst
						) / (100 + int(gold_gst_rate))
						...
						customer_metal_purity = frappe.db.sql(
							f"""select metal_purity from `tabMetal Criteria` where parent = '{self.customer}' and metal_type = '{f.metal_type}' and metal_touch = '{f.metal_touch}'""",
							as_dict=True,
						)[0]["metal_purity"]
						calculated_gold_rate = (
						...
						if f.is_customer_item:
							...
							if (
								...
								if billing_currency == "USD":
									...
									f.rate = f.se_rate
									f.making_rate = (
										find_data.get("supplier_fg_purchase_rate")
										* exchange_rate
									)
								else:
									f.rate = f.se_rate
								...
								finding_weight = getattr(
									doc, "metal_and_finding_weight", None
								)
					...
					doc.total_finding_amount = sum(
						float((r.amount)) for r in doc.get("finding_detail", [])
					)
					doc.total_finding_making_amount = sum(
						float((r.making_amount)) for r in doc.get("finding_detail", [])
					)
					doc.total_finding_wastage_amount = sum(
						float((r.wastage_amount)) for r in doc.get("finding_detail", [])
					)
				...
				if hasattr(doc, "diamond_detail"):
					for d in doc.diamond_detail:
						...
						if (
							...
							""",
								(refrence_customer, d.stone_shape),
								as_dict=True,
							)
						else:
							customer_diamond_list = frappe.db.sql(
								...
								WHERE parent = %s AND diamond_shape = %s
								""",
								(self.customer, d.stone_shape),
								as_dict=True,
							)
						# frappe.throw(f"{refrence_customer},{customer_diamond_list}")
						rate = 0
						if customer_diamond_list:
							price_list_type = customer_diamond_list[0][
								"diamond_price_list"
							]
							...
							if price_list_type == "Sieve Size Range":
								...
								common_conditions = " AND ".join(
									[f"{k} = %s" for k in common_filters.keys()]
								)
								rate_result = frappe.db.sql(
									f"""
													SELECT
							...
							if latest:
								...
								out_pct = latest.get(
									"outright_handling_charges_in_percentage", 0
								)
								work_rate = latest.get(
									"outwork_handling_charges_rate", 0
								)
								work_pct = latest.get(
									"outwork_handling_charges_in_percentage", 0
								)
								is_cust = getattr(
									d, "is_customer_item", False
								)  # Or fetch from appropriate BOM row object if accessible
								...
								if price_list_type == "Size (in mm)":
									...
									multiplier = (
										d.quantity
									)  # Or sieve size dependent multiplier
								else:  # Weight in cts
							...
							if (
								...
								# frappe.throw(f"{latest}")
								d.fg_purchase_rate = (
									latest.get("supplier_fg_purchase_rate")
									if latest
									else 0
								)
								d.total_diamond_rate = d.fg_purchase_rate
							...
							# frappe.throw(f"{price_list_type}")
								...
								d.quantity = round(d.quantity, 3)
								d.handling_rate = (
									latest.get("outright_handling_charges_rate")
									if latest
									else 0
								)
				...
				if ccp:
					...
					if doc.diamond_weight <= ccp.wt_threshold:
						...
						doc.certification_amount = ccp.per_carat_rate * (
							sum(row.quantity_3 for row in doc.diamond_detail)
						)
				...
				doc.making_charge = sum(
					row.making_amount for row in doc.metal_detail
				) + sum(row.making_amount for row in doc.finding_detail)
				# self.total=0
				...
				doc.metal_weight = doc.custom_total_metal_weight2_digits
				doc.diamond_weight = sum(row.quantity for row in doc.diamond_detail)
				doc.total_diamond_weight_in_gms = round(
					sum(row.quantity for row in doc.diamond_detail) / 5, 2
				)
				doc.total_gemstone_weight = sum(
					row.quantity for row in doc.gemstone_detail
				)
				doc.custom_total_gemstone_weight2_digits = sum(
					row.quantity_3 for row in doc.gemstone_detail
				)
				doc.gemstone_weight = doc.custom_total_gemstone_weight2_digits
				doc.total_gemstone_weight_in_gms = round(
					sum(row.quantity for row in doc.gemstone_detail) / 5, 2
				)
				doc.finding_weight = sum(row.quantity for row in doc.finding_detail)
				doc.custom_finding_weight2_digits = sum(
					row.quantity_3 for row in doc.finding_detail
				)
				doc.finding_weight_ = doc.custom_finding_weight2_digits
				doc.total_finding_weight_per_gram = doc.finding_weight
				doc.total_diamond_pcs = sum(
					float(row.pcs) for row in doc.diamond_detail
				)
				doc.total_gemstone_pcs = sum(
					float(row.pcs) for row in doc.gemstone_detail
				)
				...
				doc.total_diamond_amount = sum(
					row.diamond_rate_for_specified_quantity or 0.0
					for row in doc.diamond_detail
				)
				# doc.diamond_bom_amount = sum(row.diamond_rate_for_specified_quantity for row in doc.diamond_detail)
				doc.diamond_bom_amount = sum(
					row.diamond_rate_for_specified_quantity or 0.0
					for row in doc.diamond_detail
				)
				...
				# dharm 3-3-2026
				doc.metal_and_finding_weight = round(
					float(doc.metal_weight) + float(doc.finding_weight), 3
				)
				# frappe.throw(f"{doc.metal_weight}||{doc.finding_weight}")
				doc.gold_to_diamond_ratio = (
					float(doc.metal_and_finding_weight) / float(doc.diamond_weight)
					if doc.diamond_weight
					else 0
				)
				doc.diamond_ratio = (
					float(doc.diamond_weight) / float(doc.total_diamond_pcs)
				...
				doc.metal_to_diamond_ratio_excl_of_finding = (
					float(doc.metal_weight) / float(doc.diamond_weight)
					if doc.diamond_weight
					else 0
				)
				...
				doc.custom_total_pure_weight = sum(
					row.quantity * (float(row.metal_purity) / 100)
					for row in doc.metal_detail
				)
				doc.custom_total_pure_finding_weight = sum(
					row.quantity * (float(row.metal_purity) / 100)
					for row in doc.finding_detail
				)
				...
				total_amount = (
					...
					+ doc.custom_duty_amount
					+ doc.hallmarking_amount
					+ doc.freight_amount
					+ doc.sale_amount
				)
				...
				metal_weight = sum(
					CU.quantity for CU in doc.metal_detail if CU.is_customer_item == 0
				)
				finding_weight = sum(
					CU.quantity for CU in doc.finding_detail if CU.is_customer_item == 0
				)
				diamond_weight = (
					sum(
						CU.quantity
						for CU in doc.diamond_detail
						if CU.is_customer_item == 0
					)
				) * 0.2
				gemstone_weight = (
					sum(
						CU.quantity
						for CU in doc.gemstone_detail
						if CU.is_customer_item == 0
					)
				) * 0.2
				row.custom_company_rm_weight = (
					metal_weight + finding_weight + diamond_weight + gemstone_weight
				)
				metal_weight_ci = sum(
					CU.quantity for CU in doc.metal_detail if CU.is_customer_item == 1
				)
				finding_weight_ci = sum(
					CU.quantity for CU in doc.finding_detail if CU.is_customer_item == 1
				)
				diamond_weight_ci = (
					sum(
						CU.quantity
						for CU in doc.diamond_detail
						if CU.is_customer_item == 1
					)
				) * 0.2
				gemstone_weight_ci = (
					sum(
			...
			if self.sales_type != "Branch Sales":
							...
							if (
								...
								if gem.is_customer_item:
									...
									gem.total_gemstone_rate = (
										gpc[0]["rate"] if gpc[0]["rate"] else 0
									)
								gem.total_gemstone_rate = round(
									gem.total_gemstone_rate, 2
								)
				...
				if hasattr(doc, "metal_detail"):
					for s in doc.metal_detail:
						...
						if not mc:
							frappe.throw(
								f"""Create a valid Making Charge Price for Customer: {filters["customer"] }, Metal Type:{s.metal_touch} "Setting Type":{doc.setting_type} """
							)
				...
				if hasattr(doc, "finding_detail") and doc.finding_detail:
					...
					for f in doc.finding_detail:
						...
						if not mc:
							frappe.throw(
								f"""Create a valid Making Charge Price for Customer: {filters["customer"] }, Metal Type:{f.metal_touch} "Setting Type":{doc.setting_type} """
							)
					...
					for d in doc.diamond_detail:
						...
						if customer_diamond_list:
							...
							if 0.001 < (d.quantity / d.pcs) > 0.005:
								if (
									len(str(d.weight_per_pcs)) > 4
									and str(d.weight_per_pcs)[4] == "9"
								):
							...
							if latest:
								...
								d.handling_rate = (
									handling_rate if not is_cust else total_rate
								)
							if (
								self.company == "KG GK Jewellers Private Limited"
								and customer_group == "Internal"
								...
								d.total_diamond_rate = (
									round(base_rate, 2) if latest else 0
								)
								d.quantity = round(d.quantity, stone_precision)
								d.quantity_3 = round(d.quantity, 2)
								...
								if 0.001 < (d.quantity / d.pcs) > 0.005:
									if (
										...
										d.weight_per_pcs = float(
											str(d.weight_per_pcs)[:5]
										)
									else:
										d.weight_per_pcs = round(d.quantity / d.pcs, 3)
								...
								d.diamond_rate_for_specified_quantity = round(
									d.quantity
									* (d.total_diamond_rate + d.handling_rate),
									2,
								)
				doc.diamond_weight = sum(row.quantity for row in doc.diamond_detail)
				...
				doc.total_bom_amount = (
					...
					+ (
						sum(
							float(r.wastage_amount)
							for r in doc.get("finding_detail", [])
						)
					)
				)
				# frappe.throw(f"{doc.total_bom_amount}")
				...
				doc.metal_and_finding_weight = round(
					float(doc.metal_weight) + float(doc.finding_weight), 2
				)
				doc.gold_to_diamond_ratio = (
					float(doc.metal_and_finding_weight) / float(doc.diamond_weight)
					if doc.diamond_weight
					...
					if not metal.is_customer_item:
						...
						for e_item in e_invoice_items:
							if (
								...
								multiplied_qty = metal.quantity * item.qty
								metal_making_amount = (
									metal.making_rate * multiplied_qty
