	...
	if tolerance:
		...
		for tolerance_row in customer_tolerance:
			if (
				...
				and tolerance_row.get("range_type") == "Prcentage"
			):
...
def download_cost_sheet(sales_order):
	...
	if customer_name == "Caratlane Trading Private Limited":
		...
		diamond_quality_names = [
			row["diamond_quality"] for row in customer_diamond_qualitys
		]
		num_dq_cols = len(
			diamond_quality_names
		...
		for row in so_doc.items:
			...
			if mg in ["E", "F"]:
				...
				if making_charge:
					...
					rate_per = (
						net_weight * making_charges[0].get("rate_per_gm")
						if making_charges
						else ""
					)
					rate_per_pc = (
	...
	if customer_name.strip() == "Titan Company Limited":
		...
		for row in so_doc.items:
			...
			if making_charge:
				making_charge_rate = frappe.db.get_value(
					"Making Charge Price Item Subcategory",
					{"parent": making_charge, "subcategory": item_sub_category},
					["rate_per_gm"],
				)
				making_charge_wastage = frappe.db.get_value(
					"Making Charge Price Item Subcategory",
					{"parent": making_charge, "subcategory": item_sub_category},
					["wastage"],
				)
			...
			for pcs in diamond_pcs:
				...
				handling_rate = (
					...
					* pcs.get("quantity" or 0)
					if diamond_rate[0].get("outright_handling_charges_rate")
					else 0
				)
				diamond_rt = (
					diamond_rate[0].get("rate") * pcs.get("quantity" or 0)
					if diamond_rate[0].get("rate")
					else 0
				)
			...
			total = (
				(
					(net_weight * making_charge_wastage) / 100
					if making_charge_rate
					else 0
				)
