...
def customer_tolerance(customer, weight):
	...
	if tolerance:
		...
		for tolerance_row in customer_tolerance:
			if (
				tolerance_row.get("range_type")
				and tolerance_row.get("range_type") == "Percentage"
			):
				return tolerance_row.tolerance_range
			if (
				weight >= tolerance_row.from_weight
				and weight <= tolerance_row.to_weight
			):
				return tolerance_row.tolerance_range
	return 0
...
def customer_making_details(customer, metal_rate_gst, metal_rate, bom_doc):
	...
	if not (
		metal.get("quantity") and metal.get("metal_type") and metal.get("metal_touch")
	):
		return 0, 0, 0, 0
	...
	making_rate = (
		flt(sub_info.rate_per_pc) if is_below_threshold else flt(sub_info.rate_per_gm)
	)
	...
	calculated_gold_rate = (flt(customer_metal_purity) * metal_rate_gst) / (
		100 + metal_rate
	)
...
def download_cost_sheet(sales_order):
	...
	if customer_name == "Caratlane Trading Private Limited":
		...
		diamond_quality_fill = PatternFill(
			start_color="FFF2CC", end_color="FFF2CC", fill_type="solid"
		)  # Light yellow for diamond quality columns
		...
		customer_diamond_qualities = frappe.get_all(
		...
		diamond_quality_names = [
			row["diamond_quality"] for row in customer_diamond_qualities
		]
		num_dq_cols = len(
			diamond_quality_names
		)  # Number of dynamic diamond quality columns
		...
		for soi in so_doc.items:
			...
			try:
				if bom.get("product_size") and bom.get("sizer_type"):
					product_size = (
						f"{bom.get('product_size')} {bom.get('sizer_type')}".strip()
					)
				if not product_size:
				...
				frappe.log_error(
					...
					"sales_order_utils: product_size lookup failed",
				)
				product_size = ""
			...
			for idx, stone in enumerate(stone_rows):
				...
				row_data[23] = (
					row_data[22] * row_data[16] if row_data[22] and row_data[16] else ""
				)
				...
				if is_first_row:
					...
					tolerance_amount_without_finding = (row_data[28] or 0) * (
						tolerance_range / 100
					)
					row_data[36] = row_data[28] or 0
					row_data[35] = (
						(row_data[28] or 0) - tolerance_amount_without_finding
					) or ""
					row_data[37] = (
						(row_data[28] or 0) + tolerance_amount_without_finding
					) or ""
					...
					tolerance_amount_with_finding = (row_data[39] or 0) * (
						tolerance_range / 100
					)
					row_data[38] = (
						(row_data[39] or 0) - tolerance_amount_with_finding
					) or ""
					row_data[40] = (
						(row_data[39] or 0) + tolerance_amount_with_finding
					) or ""
					...
					row_data[46] = (
						"Yes"
						if row_data[33] and row_data[33].lower() == "14kt"
						else "No"
					)
					row_data[47] = (
						"Yes"
						if row_data[33] and row_data[33].lower() == "18kt"
						else "No"
					)
					row_data[48] = (
						"Yes"
						if row_data[33] and row_data[33].lower() == "22kt"
						else "No"
					)
					row_data[49] = (
						"Yes"
						if row_data[33] and row_data[33].lower() == "platinum"
						else "No"
					)
					row_data[50] = (
						"Yes"
						if row_data[7] and row_data[7].lower() == "white"
						else "No"
					)
					row_data[51] = (
						"Yes"
						if row_data[7] and row_data[7].lower() == "yellow"
						else "No"
					)
					row_data[52] = (
						"Yes" if row_data[7] and row_data[7].lower() == "pink" else "No"
					)
					...
					mc = frappe.get_all(
						...
						filters={
							...
							"making_components": serial_no_data.get(
								"custom_making_component"
							),
						},
						fields=["name"],
						limit=1,
					...
					row_data[COL_METAL_VAL] = (
						metal.get("amount") if bom.metal_detail else ""
					)
					row_data[COL_LABOR] = (
						lbr_amt[0].get("rate_per_gm") if lbr_amt else ""
					)
					row_data[COL_PER_PC_LAB] = (
						lbr_amt[0].get("rate_per_pc") if lbr_amt else ""
					)
					row_data[COL_TOTAL_DIA] = flt(bom.diamond_weight)
					row_data[COL_TOTAL] = (
						bom.metal_and_finding_weight * lbr_amt[0].get("rate_per_gm")
						if lbr_amt
						else 0
					)
					row_data[COL_TOTAL_PRICE] = (
						metal.get("amount") if bom.metal_detail else 0
					) + (
						bom.metal_and_finding_weight * lbr_amt[0].get("rate_per_gm")
						if lbr_amt
						else 0
					...
					if metal_rate_gst:
						customer_price_data = customer_making_details(
							customer, metal_rate_gst, metal_rate, bom
						)
				...
				for i, quality_name in enumerate(diamond_quality_names):
					col_idx = DQ_START + i
					row_data[col_idx] = (
						"Yes" if stone_quality_raw == quality_name else "No"
					)
		...
		for col_idx in range(1, total_cols + 1):
			...
			max_line_len = max(
				(len(line) for line in str(header_val).split("\n")), default=8
			)
	...
	if customer_name == "Novel Jewels Limited":
		...
		headers = [
			...
			"Rate",
			"Value",
			"Labour/ Dia Handling / Other per ct or gm charges",
			"Labour Amt",
			"Loss / CS Handling / Other %",
			"Loss Amt",
			"Stone Shape",
			"Setting Type",
			"Stone Cut/ Cab",
		...
		yellow_fill = PatternFill(
			start_color="FFF200", end_color="FFF200", fill_type="solid"
		)  # Yellow
		...
		for row in so_doc.items:
			...
			if diamond_qlty and "-" in diamond_qlty:
				fg, si = diamond_qlty.split("-", 1)
			else:
				fg = ""
				si = ""

			bom_finding = frappe.get_all(
				"BOM Finding Detail",
			...
			category = frappe.get_all(
				...
				fields=["customer_subcategory", "customer_category"],
				limit_page_length=1,
			)

			digit_code = frappe.get_all(
				"Item Theme Code Detail",
				filters={
					"customer": so_doc.customer,
					"parent": row.get("item_code"),
				},
				fields=["theme_code", "digit_15code"],
				limit_page_length=1,
			)

			metal_detail = frappe.get_all(
				"BOM Metal Detail", filters={"parent": row.get("bom")}, fields=["*"]
			)
			prodct_size = frappe.db.get_value(
				"Novel Size Master",
				{
					"customer": so_doc.customer,
			...
			mg = frappe.db.get_value(
				"Serial No",
				{"name": row.get("serial_no")},
				"custom_mfg_complexity_code",
			)
			mg_code = frappe.db.get_value(
				"Complexity Category",
				{"mfg_code": mg, "parent": "MHCU0008"},
				"complexity_name",
			)

			making_charge = frappe.db.get_all(
				"Making Charge Price",
				filters={
					"customer": so_doc.customer,
			...
			if mg in ["E", "F"]:
				if making_charge:
					making_charges = frappe.db.get_all(
						"Making Charge Price Item Subcategory",
						filters={
					...
					rate_per = (
						making_charges[0].get("rate_per_pc") if making_charges else ""
					)
					rate_per_pc = (
						making_charges[0].get("rate_per_pc") if making_charges else ""
					)
				...
				if making_charge:
					...
					rate_per = (
						flt(net_weight) * flt(making_charges[0].get("rate_per_gm"))
						if making_charges
						else ""
					)
					rate_per_pc = (
						making_charges[0].get("rate_per_gm") if making_charges else ""
					)
				# frappe.throw(str(making_charge))
			...
			# 		making_code = 'C'
			total_value = (
				total_value + metal_detail[0].get("amount") if metal_detail else 0
			)
			lbr_amount = (
				lbr_amount + metal_detail[0].get("making_amount") if metal_detail else 0
			)
			wastage_amt = (
				wastage_amt + metal_detail[0].get("wastage_amount")
				if metal_detail
				else 0
			)
			row_data = [
				...
				"",
				"",
				(metal_detail[0].get("amount") if metal_detail else 0)
				+ flt(rate_per)
			...
			# -------------------- DIAMOND DETAILS --------------------
			diamond_detail = frappe.get_all(
				"BOM Diamond Detail", filters={"parent": row.get("bom")}, fields=["*"]
			)
			total_se_rate = 0
			...
			for item in diamond_detail:
				...
				total_value += (
					item.get("quantity", 0) * diamond_rate[0].get("rate", 0)
					if diamond_rate
					else 0
				)
				lbr_amount += (
					item.get("quantity", 0)
					* diamond_rate[0].get("outright_handling_charges_rate", 0)
					if diamond_rate
					else 0
				)
				row_data = [
					...
					item.get("quantity") * diamond_rate[0].get("rate")
					if diamond_rate
					else 0,
					diamond_rate[0].get("outright_handling_charges_rate")
					if diamond_rate
					else "",
					item.get("quantity")
					* diamond_rate[0].get("outright_handling_charges_rate")
			...
			# -------------------- GEMSTONE DETAILS --------------------
			gemstone_detail = frappe.get_all(
				"BOM Gemstone Detail", filters={"parent": row.get("bom")}, fields=["*"]
			)
			total_se_rate = 0
			for item in gemstone_detail:
				...
				gemstone_rate = None
				if stone_code:
					gemstone_rate = frappe.db.get_all(
						"Gemstone Price List",
						filters={
							...
							"gemstone_code": stone_code[0].get("customer_code")
							if stone_code
							else "",
						},
						fields=["outright_handling_charges_rate", "rate"],
				...
				total_value += (
					item.get("pcs", 0) * gemstone_rate[0].get("rate", 0)
					if gemstone_rate
					else 0
				)
				row_data = [
					...
					item.get("pcs") * gemstone_rate[0].get("rate")
					if gemstone_rate
					else 0,
	...
	if customer_name.strip() == "Titan Company Limited":
		...
		for row in so_doc.items:
			...
			digit_14code = frappe.db.get_value(
				"Item Theme Code Detail",
				{"customer": so_doc.customer, "parent": row.item_code},
				"digit_14code",
			)

			making_charge = frappe.db.get_value(
				"Making Charge Price",
				{
					"customer": so_doc.customer,
					"metal_touch": metal_touch,
			...
			if making_charge:
				making_charge_data = (
					frappe.db.get_value(
						"Making Charge Price Item Subcategory",
						{"parent": making_charge, "subcategory": item_sub_category},
						["rate_per_gm", "wastage"],
						as_dict=True,
					)
					or {}
				)
				making_charge_rate = making_charge_data.get("rate_per_gm")
				making_charge_wastage = making_charge_data.get("wastage")
			...
			for pcs in diamond_pcs:
				...
				handling_rate = (
					diamond_rate[0].get("outright_handling_charges_rate")
					* pcs.get("quantity" or 0)
					if diamond_rate
					and diamond_rate[0].get("outright_handling_charges_rate")
					else 0
				)
				diamond_rt = (
					diamond_rate[0].get("rate") * pcs.get("quantity" or 0)
					if diamond_rate and diamond_rate[0].get("rate")
					else 0
				)
			...
			for gems in gemstone:
				...
				gemstone_detail_rows.append(
					[
						...
						setting_type,
						gemstone_rate[0].get("rate") if gemstone_rate else "",
						gems.get("pcs" or 0) * gemstone_rate[0].get("rate")
						if gemstone_rate
						else "",
						gems.get("pcs" or 0)
						* gemstone_rate[0].get("outright_handling_charges_rate")
			...
			total = (
				(
					(flt(net_weight) * flt(making_charge_wastage)) / 100
					if making_charge_wastage is not None
					else 0
				)
				+ (
					flt(net_weight) * flt(making_charge_rate)
					if making_charge_rate is not None
					else 0
				)
				+ total_handling
			)
