...
#: This exclusion is why 22,140 g of customer metal carries ``pure_qty = 0`` -- the rows were
#: never reached, so the zeros are "never computed", not "computed as zero". Nothing was wrong
#: with the inputs: the 24KT item carries its Metal Purity attribute and every Manufacturing
#: Setting resolves ``pure_gold_item``.
...
def _pure_qty_excluded_types():
	...
	exclusion list so customer receipts finally get a real pure quantity -- Day 6's balance
	calculation, Day 8's allocation and Day 12's per-serial split all read it, and a wrong
	zero would propagate into every one of them.
