...
def before_validate(self, method):
	...
	for row in self.items:
		if (
			...
			and row.manufacturing_operation
...
def validate_metal_properties(doc):
	...
	for row in doc.items:
		...
		if not item_data.get(row.item_code):
			...
			item_data[row.item_code]["mop"] = (
				[row.manufacturing_operation] if row.manufacturing_operation else []
			)
			...
			if row.custom_manufacturing_work_order not in item_data[row.item_code]["mwo"]:
				item_data[row.item_code]["mwo"].append(row.custom_manufacturing_work_order)

			if main_slip not in item_data[row.item_code]["mop"]:
				item_data[row.item_code]["mop"].append(main_slip)
	...
	for item in item_data:
		for mwo in item_data[item]["mwo"]:
			...
			if (
				...
				and mwo_data.metal_colour != item_data[item].metal_colour
				...
				if not msl_data.get("for_subcontracting"):
					...
					if msl_data.metal_colour:
						...
						if company_validations.get("check_colour"):
							if msl_data.metal_colour != item_data[item].metal_colour and msl_data.check_color:
	...
	for row in mwo_erros:
		combine_components = ", ".join(mwo_erros[row])
	...
	for row in msl_erros:
		combine_components = ", ".join(msl_erros[row])
		if combine_components:
			all_error_msg.append(
		...
		if entry.manufacturing_operation:
			...
			if is_cancelled:
				...
				for doctype in [
					...
					if frappe.db.exists(doctype, {"sed_item": entry.name}):
						to_remove.append(frappe.db.get_value(doctype, {"sed_item": entry.name}, "name"))
				...
				d_warehouse, e_warehouse = get_warehouse_details(
					mop_basic_details[entry.manufacturing_operation], se_employee, se_subcontractor
				)
