...
function set_filters_ct_fields(frm){
	...
	frappe.db.get_list('Tolerance Weight Type', {
        ...
        console.log(filter_for_gemstone ,"gem", filter_for_metal,"metal",filter_for_diamond,"non metal");
		frm.fields_dict['metal_tolerance_table'].grid.get_field('weight_type').get_query = function (doc, cdt, cdn) {
			return {
				filters: [
					[`name`, `in`, filter_for_metal]
				]
			};
		};
		frm.fields_dict['diamond_tolerance_table'].grid.get_field('weight_type').get_query = function (doc, cdt, cdn) {
			return {
				filters: [
					[`name`, `in`, filter_for_diamond ]
				]
			};
		};
		frm.fields_dict['gemstone_tolerance_table'].grid.get_field('weight_type').get_query = function (doc, cdt, cdn) {
			return {
				filters: [
					[`name`, `in`, filter_for_gemstone]
				]
			};
		};
	})
...
function toggle_diamond_table_field(frm,cdt,cdn){
    let d = locals[cdt][cdn]
    let mm_size_wise = ['diamond_type','sieve_size','from_diamond','to_diamond','plus_percent','minus_percent'];
    let group_size_wise = ['diamond_type','sieve_size_range','from_diamond','to_diamond','plus_percent','minus_percent'];
    let weight_wise = ['diamond_type','from_diamond','to_diamond','plus_percent','minus_percent'];
    let universal = ['plus_percent','minus_percent']
    let all_field = ['diamond_type','sieve_size','sieve_size_range','from_diamond','to_diamond','plus_percent','minus_percent']
