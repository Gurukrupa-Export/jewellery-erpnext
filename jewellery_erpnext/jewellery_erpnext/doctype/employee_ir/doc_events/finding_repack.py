...
module turns that into stock:
  ...
  2. The tree's ``material_details`` ledger is charged for what was consumed (``receive_qty`` and
     ``wo_receive_qty``), so ``pending_qty`` drops by exactly the finding weight -- a tree with
     14 g pending that yields 2 g of findings is left with 12 g.
...
def _charge_tree(tree, metal_item, qty, prec):
	...
	``receive_qty`` and ``wo_receive_qty`` move in lockstep, the same pair
	``tree_casting.update_tree_on_receive`` writes -- this IS a work-order-driven draw, so it
	belongs to the work-order half of the receive provenance split.

	``pending_qty`` and ``manual_receive_qty`` are deliberately NOT written here: both are derived
	on every save by ``TreeNumber.validate`` (``calculate_material_pending`` /
	``derive_manual_receive``), and a second writer is exactly what used to make the four call
	paths drift apart.
	"""
	if flt(qty, prec) <= 0:
		return
	...
	md.wo_receive_qty = flt(flt(md.wo_receive_qty) + flt(qty), prec)
...
def reverse_finding_draw_on_trees(eir):
	...
	for tree_name in sorted(draws):
		...
		for metal_item, qty in draws[tree_name].items():
			...
			md.wo_receive_qty = max(0.0, flt(flt(md.wo_receive_qty) - flt(qty), prec))
		tree.status = tree_balance.tree_status(tree)
		tree.flags.ignore_permissions = True
		tree.save()
