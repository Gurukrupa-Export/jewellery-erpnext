	...
	def on_submit_receive(self, cancel=False):
		...
		for row in self.employee_ir_operations:
			...
			if not cancel:
				...
				res["complete_time"] = frappe.utils.now()
				frappe.db.set_value(
			...
			if frappe.db.get_value(
				...
				if not cancel:
					se_rows, msl_rows, product_loss = create_stock_entry(self, row, flt(difference_wt, precision), existing_msl)
				...
				if row.employee and not operation_data.get(row.manufacturing_operation):
					if not mop_data.get(row.manufacturing_operation):
						mop_data[row.manufacturing_operation] = frappe.db.get_value(
							"Manufacturing Operation",
							row.manufacturing_operation,
							["time_taken", "manufacturing_order"],
							as_dict=1,
						)
					...
					operation_data[row.manufacturing_operation] = {
						...
						"operation_time": mop_data[row.manufacturing_operation].time_taken or 0,
			...
			if operation_data:
				for row in operation_data:
					expense_account = frappe.db.get_value(
						"Company", se_doc.company, "default_operating_cost_account"
					)
					additional_cost = {
						"expense_account": expense_account,
						"amount": operation_data[row]["total_expense"],
						"description": "Workstation Cost",
						"custom_manufacturing_operation": operation_data[row]["mop"],
					}
...
def create_operation_for_next_op(docname, target_doc=None, employee_ir=None, received_gr_wt=0):
...
def create_stock_entry(doc, row, difference_wt=0, existing_msl = None):
	metal_item = None
	...
	department_wh, employee_wh = get_warehouses(doc)
	...
	if difference_wt != 0:
		...
		if difference_wt < 0 and (metal_item not in existing_items):
			...
			if loss_items:
				process_loss_rows += process_loss_entry(doc, loss_items, employee_wh, department_wh)
		elif doc.main_slip:
			pure_ms_qty = 0
			# Loss done through Manual Table + Loss Table
			if loss_items and difference_wt < 0:
				process_loss_rows += process_loss_entry(doc, loss_items, employee_wh, department_wh)

			msl_raw_warehouse = frappe.db.get_value("Main Slip", doc.main_slip, "raw_material_warehouse")
			...
			if difference_wt > 0:
				...
				for b_id in ms_batch:
					existing_msl.setdefault(b_id.batch_no, {"total_qty" : flt(b_id.qty - b_id.consume_qty, 3), "utilized_qty": 0})
					if b_id.qty <= b_id.consume_qty and existing_msl[b_id["utilized_qty"]] >= flt(b_id.qty - b_id.consume_qty, 3):
						continue
					...
					if remaining_wt > 0:
						...
						if (b_id.consume_qty + remaining_wt) <= b_id.qty:
							...
							remaining_wt -= se_qty
						existing_msl[b_id["utilized_qty"]] += se_qty

						msl_rows.append(
							{
								"item_code": metal_item,
								"s_warehouse": msl_raw_warehouse if difference_wt > 0 else employee_wh,
								"t_warehouse": msl_raw_warehouse if difference_wt < 0 else employee_wh,
								"to_employee": None,
	...
	return se_rows, msl_rows, process_loss_rows
...
def process_loss_entry(doc, manual_loss_items, employee_wh, department_wh):
	process_loss_row = []
	...
	for loss_item in manual_loss_items:
		if abs(loss_item.get("loss_qty")):
			if doc.main_slip and loss_item.get("variant_of") in ["M", "F"]:
				...
				process_loss_row += process_loss_item(doc, loss_item, employee_wh, department_wh)

	return process_loss_row


def process_loss_item(doc, loss_item, employee_wh, loss_warehouse):
	process_loss_row = []
...
def create_single_se_entry(doc, mop_data):
	...
	for row in mop_data:
		rows_to_append += get_rows_to_append(doc, row, mop_data[row], department_wh, employee_wh)
...
def get_rows_to_append(doc, mwo, mop, department_wh, employee_wh):
	rows_to_append = []
	import copy
