...
class ManufacturingPlan(Document):
    def on_submit(self):
        ...
        if is_subcontracting:
            # Throws when the source Sales Orders disagree: the values are about to be stamped onto
            # a supplier's Purchase Order, and one wrong value there is not recoverable.
            source_order_dimensions(self, strict=True)
    ...
    def set_order_dimensions(self):
        """Stamp order type / sales type / flow type from the source Sales Orders onto the header.

        Left blank when the source orders disagree, so the plan stays editable; `on_submit` throws
        for a subcontracting plan, where the values have to ride onto a Purchase Order.
        """
        ...
        for row in self.manufacturing_plan_table:
            ...
            if not row.manufacturing_bom and frappe.db.exists("BOM", row.bom):
                if not bom_type_data.get(row.bom):
                    bom_type_data[row.bom] = frappe.get_value(
                        "BOM", {"name": row.bom}, "bom_type"
                    )
                ...
                if bom_type == "Sales Order":
                    ...
                    frappe.throw(
                        _(
                            "Manufactuing Bom Creation Error: {0} BOM Type Must be a Sales Order."
                        ).format(row.bom)
                    )
        for row in bom_data:
    ...
    def get_sales_orders(self):
        ...
        query = (
            ...
            .on(
                (SalesOrderItem.parenttype == "Sales Order")
                & (SalesOrderItem.parent == SalesOrder.name)
            )
            .select(SalesOrder.name)
            .where(
                (SalesOrder.docstatus == 1.0)
        ...
        if not self.manufacturing_work_order:
            ...
            for item_row in items:
                ...
                if so_bom or item_master_bom:
                    check_bom_type = frappe.get_value(
                        "BOM", so_bom or item_master_bom, "bom_type"
                    )
                    if check_bom_type == "Sales Order":
                        item_row["manufacturing_order_qty"] = item_row.get(
                            "pending_qty"
                        )
            ...
            self.manufacturing_plan_table = []
            mwo_lst = [
                row.manufacturing_work_order for row in self.manufacturing_work_order
            ]
