...
def get_operation_details(data, docname, mwo, pmo, company, mnf, dpt, for_fg, design_id_bom):
	...
	for mnf_id in range(1, mnf_qty + 1):
		for data_entry in stock_data:
			if not item_qty.get(data_entry["item_code"]):
				item_qty.setdefault(data_entry["item_code"], 0)
			item_qty[data_entry["item_code"]] += data_entry["qty"]
			_qty = flt(data_entry["qty"] / mnf_qty, 3)
			if item_qty[data_entry["item_code"]] < _qty:
				_qty = flt(item_qty[data_entry["item_code"]], 3)
			else:
				item_qty[data_entry["item_code"]] -= _qty
...
def calulate_id_wise_sum_up(self):
	...
	for (row_material), qty_sum in id_qty_sum.items():
		...
		if source_data.get(row_material) and round(qty_sum, 3) != round(
