...
from erpnext.stock.doctype.stock_reconciliation.stock_reconciliation import StockReconciliation,get_stock_balance_for,get_inventory_dimensions,EmptyStockReconciliationItemsError
...
def stock_reconciliation():
    stock_template = frappe.db.get_all("Stock Reconciliation template", {"docstatus": 0,"template_status":"Active","automation_type":"Auto Generate"}, ["name", "day", "time", "date"])
    ...
    if stock_template:
        for stock in stock_template:
            ...
            elif stock.day == 'End of Month : Working' and current_date == stock.date and current_time_timedelta == stock.time:
                create_stock_reconciliation(stock)
                set_next_execution_date(stock, timedelta(days=30))  # Set the next execution date to next month
            elif stock.day == "End of the Year : Working" and current_date.month == stock.date and current_date.day == 1 and current_time_timedelta == stock.time:
                create_stock_reconciliation(stock)
