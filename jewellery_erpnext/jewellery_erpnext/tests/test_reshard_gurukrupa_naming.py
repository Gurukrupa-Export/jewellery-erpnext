...
from jewellery_erpnext.patches import reshard_gurukrupa_stock_entry_naming as reshard_mod
...
class TestBindsToCompany(IntegrationTestCase):
	...
	def test_a_missing_operator_does_NOT_bind(self):
		...
		self.assertFalse(
			reshard_mod._binds_to_company([frappe._dict(field="company", value=_COMPANY)], _COMPANY)
		)
...
class TestTargetRulesRespectsTheOperator(IntegrationTestCase):
	...
	def test_disabled_selector_skips_a_not_equals_rule(self):
		out, _ = self._run(
			...
			{"name": "R1", "prefix": "GE-SE-X-.YY.-", "prefix_digits": 5, "counter": 0, "priority": 0},
			[_cond("company", "!=", _COMPANY)],
		)
	...
	def test_enabled_selector_skips_a_not_equals_rule(self):
		...
		self.assertEqual(out, [], "a `company != X` rule must never be disabled by rollback")
