...
class TestConflictKey(IntegrationTestCase):
	@classmethod
	def setUpClass(cls):
		pass
	...
	def test_prefix_and_join(self):
		self.assertEqual(
			conflict_key("bin", "ITEM", "WH"), "jewl_conflict_bin__ITEM__WH"
		)
...
class TestConflictLock(IntegrationTestCase):
	@classmethod
	def setUpClass(cls):
		pass
	...
	def test_acquires_filelock_with_derived_name_and_timeout(self):
		...
		with patch.object(serialize, "filelock", _fake):
			with conflict_lock("bin", "ITEM", "WH", timeout=15):
				pass
		self.assertEqual(_fake.called_with, ("jewl_conflict_bin__ITEM__WH", 15))
