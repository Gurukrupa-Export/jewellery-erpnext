...
A finished piece carries no indication of what Order Type it was sold under.
``Serial Number Creator`` already fetches ``order_type`` from
``parent_manufacturing_order`` (which in turn traces back to the Sales Order /
Quotation), so the value is available by the time ``create_manufacturing_entry``
(manufacturing_operation.py) stamps the other derived fields
(``custom_product_type`` / ``custom_gross_wt`` / ``custom_repair_type``) onto the
newly created Serial No, right after the Manufacture Stock Entry is submitted.
This field is stamped alongside them there.
...
Wired in the two idempotent places this app uses for custom fields (this
``post_model_sync`` patch and ``create_test_data.setup_data``), for the
usual reason: ``after_migrate`` is disabled and ``install-app`` marks patches
		...
		"Serial No": [
			{
				...
				"options": "\nSales\nMaintenance\nShopping Cart\nStock Order\nRepair",
				# Anchored on the STANDARD ``customer`` field — the anchor the retired
				# ``custom_ownership_tag`` used — so the layout does not depend on
				# another app's Serial No fixtures being installed.
