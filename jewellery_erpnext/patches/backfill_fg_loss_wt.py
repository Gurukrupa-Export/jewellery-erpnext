...
loss. It now sums every loss (negative ``loss_wt``) over the sibling operations that have a
submitted Employee IR receive (``cumulative_loss_wt``). A positive value is material coming in,
such as the casting receipt, and a value on an operation nobody received is a leftover from a
cancelled receive, a Department IR copy or a split. This script applies the same rule to headers
