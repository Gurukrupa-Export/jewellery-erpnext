...
Both helpers return False silently when their target row was missing (no workflow named
"Material Request", or no System Manager Custom DocPerm row) -- every sibling patch to this
same DB-only workflow (add_mr_transfer_to_department_workflow, update_mr_department_transfer_visibility,
add_mr_department_to_mop_transition) is built on the same silent-skip contract, because a
fresh site genuinely has no such workflow until someone sets it up by hand, and throwing here
would break `bench migrate` on every one of them. Instead, the missing-row case writes to the
Error Log, so a site where this matters (one already running the Material Request workflow)
surfaces it instead of the outcome looking identical to "already correct".
