"""Move ``Quotation.custom_invoice_item`` into its own full-width section
immediately after the ``net_total`` field.

Currently the Table renders at index 101 inside ``custom_section_break_emde6``
(after ``referral_sales_partner``). This patch:
...
3. injects both fields into the ``field_order`` Property Setter immediately
   after ``net_total`` (index 65) and before ``taxes_section``,

keeping the exact same fieldname / fieldtype / options so existing
``Quotation E Invoice Item`` child rows (linked by ``parent`` = Quotation
name) remain intact.

Safe to re-run: every step is guarded by existence checks and idempotent
keyed lookups. Can be run ad-hoc::

    bench --site <site> execute jewellery_erpnext.patches.move_quotation_invoice_item.execute
"""
...
def execute():
    ...
    frappe.db.set_value(
        "Custom Field", {"dt": doctype, "fieldname": SECTION_FIELD}, "insert_after", ANCHOR
    )
    frappe.db.set_value(
    ...
    print(f"[move_quotation_invoice_item] {TABLE_FIELD} moved into {SECTION_FIELD} after {ANCHOR}")